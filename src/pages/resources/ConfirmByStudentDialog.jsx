  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const result = await uploadInstallmentDocument({
        studentPaymentInstallmentId: installment.studentPaymentInstallmentId,
        fileBase64: base64,
        fileName: file.name,
      });

      const uploadedUrl =
        result?.documentUrl ?? result?.DocumentUrl ?? result?.url ?? result?.Url;
  ...
  return (
    <Dialog open={open} onClose={handleClose} fullWidth maxWidth="sm">
      <DialogTitle sx={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
        ...
        <Button onClick={handleClose} disabled={uploading || sending} sx={{ minWidth: 0, p: 0.5 }}>
          <CloseIcon />
        </Button>
      </DialogTitle>
      ...
      <DialogContent>
        ...
        >
          ...
          <Button component="label" variant="outlined" disabled={uploading || sending}>
            Choose File
            <input
              type="file"
              hidden
        ...
        {(uploading || sending) && <LinearProgress sx={{ mt: 2 }} />}

        <Alert severity="info" sx={{ mt: 2 }}>
          Allowed file types: PDF, DOC, DOCX, JPG, PNG. Maximum size: 10MB.
        </Alert>

        {documentUrl && (
          <Alert severity="success" sx={{ mt: 2 }}>
            Document uploaded successfully. You can now send the email.
          </Alert>
        )}
      </DialogContent>

      <DialogActions sx={{ px: 3, pb: 2 }}>
        <Button onClick={handleClose} disabled={uploading || sending}>
          Cancel
        </Button>
